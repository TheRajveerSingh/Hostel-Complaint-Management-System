...
const mockStaff = [
  { id: 'S-001', name: 'Ramesh Kumar', role: 'Electrician', categories: 'Electrical, Plumbing', location: 'Hostel A', status: 'Free', complaints: 0, rating: 4.8 },
  { id: 'S-002', name: 'Suresh Patel', role: 'Plumber', categories: 'Plumbing', location: 'Hostel B', status: 'Busy', complaints: 2, rating: 4.5 },
  { id: 'S-003', name: 'Ravi Verma', role: 'Network Tech', categories: 'Internet, TV', location: 'Mess', status: 'Free', complaints: 0, rating: 4.9 }
];
...
export default function WardenStaffList() {
  ...
  const stats = [
    { label: 'Total Personnel', value: mockStaff.length, color: 'text-on-surface', icon: Users },
    { label: 'Active Duty', value: mockStaff.filter(s => s.status === 'Busy').length, color: 'text-secondary', icon: Activity },
    { label: 'Available Now', value: mockStaff.filter(s => s.status === 'Free').length, color: 'text-success', icon: CheckCircle2 },
    { label: 'Avg Efficiency', value: '94%', color: 'text-primary', icon: Zap }
  ];
                ...
                <tr key={s.id} className="hover:bg-secondary/[0.03] transition-all duration-300 group">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[10px] text-tertiary font-black uppercase tracking-widest">{s.rating} Score</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-10 py-8">
                    <div className="flex flex-wrap gap-2">
                      {s.categories.split(', ').map(cat => (
                        <span key={cat} className="text-[8px] font-black text-secondary uppercase tracking-widest py-1 px-2 bg-secondary/10 rounded-md border border-secondary/10">
                          {cat}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{s.location}</span>
                    </div>
                  </td>
                  <td className="px-10 py-8">
                    <span className={`px-4 py-2 text-[10px] uppercase tracking-[0.2em] font-black rounded-xl backdrop-blur-md border ${
                      ...
                      {s.status === 'Free' ? 'Standby' : 'Deployed'}
                    </span>
                  </td>
                  <td className="px-10 py-8 text-right">
                    <div className="flex flex-col items-end">
                      <span className="text-xs font-black text-on-surface tracking-widest uppercase">{s.complaints} Active</span>
                      <span className="text-[8px] text-on-surface-variant font-black uppercase tracking-widest opacity-40">Resolved: 42</span>
                    </div>
                  </td>
                </tr>
              ))}
